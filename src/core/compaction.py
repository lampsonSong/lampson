...
- 任务不丢失：压缩后必须注入任务进度摘要，agent 知道当前在做什么
...
DEFAULT_TRIGGER_THRESHOLD = 0.95
...
class CompactionConfig:
    ...
    keep_recent_n: int = 8               # 保留最近 N 轮不做摘要（调高以保留任务上下文）
...
class Compactor:
    ...
    def compact(
        ...
        **调用方**：Agent 运行时由 `maybe_compact()` 调用，不在 Session 退出时调用。
        ...
        # Step 8: 生成任务上下文摘要并注入（防止任务丢失）
        # 这是最关键的修复：多次压缩后 agent 仍然知道当前在做什么
        _notify_progress(progress_callback, "[上下文] 正在生成任务进度摘要...")
        ...
        if task_context:
            ...
            remaining = [context_msg] + remaining
            logger.info(f"Compaction 注入任务上下文摘要: {task_context[:80]}...")
    ...
    def _generate_task_context(self, messages: list[dict[str, Any]]) -> str:
        """生成任务进度摘要，告诉 agent 当前在做什么。

        这是防止多次压缩后任务上下文丢失的关键机制。
        摘要基于全部原始消息（包括将被归档的），确保关键信息不丢失。
        """
        # 取最近的 N 条 user 消息和 assistant 回复作为摘要素材
        # 这些是当前任务的直接上下文，不会被归档
