...
```
~/.lampson/memory/
├── core.md          # 核心记忆：用户偏好、安全规则、重要事实（全量加载）
├── sessions/        # 会话记忆：每次对话的摘要（按需检索）
│   └── 2026-04-23.md
└── working.json     # 工作记忆：当前会话上下文（自动管理）
```
...
| 会话记忆(session) | sessions/*.md | 按关键词检索 | 每文件 < 10KB |
| 工作记忆(working) | working.json | 当前会话维护，退出时摘要 | < 2KB |
...
1. **CLI 基础**: REPL 交互，支持自然语言输入
2. **LLM 对话**: 对接 OpenAI 兼容 API，支持多轮对话
3. **工具调用**: 执行 shell 命令、读写文件、文件编辑
4. **Memory**: 三层记忆（核心/会话/工作），基本增删查
5. **Skills**: 技能发现与加载，支持内置技能
6. **飞书通信**: 发送+接收消息
7. **自更新**: 基本代码自修改 + git 回滚

### 暂不做

- MCP Server 完整对接（预留接口）
- TUI 界面（用简单 REPL）
- 多用户支持
- 插件市场
- 语义搜索记忆
...
1. **LLM Provider**: MVP 先用哪个？OpenAI/Anthropic/本地模型？
2. **CLI框架**: Prompt Toolkit（简单）还是 Textual（好看但复杂）？
3. **飞书方式**: 直接 API 调用还是通过 MCP Server？
4. **包管理**: pip install -e . 还是直接 python -m？
5. **自更新边界**: 哪些核心文件不允许自修改？
