...
以下两项为已识别但暂不实施的能力增强，待后续排期。

### 12.1 用户反馈结构化

**现状**：`reflection.py` 有 `_detect_user_correction` 检测纠正信号词，触发反思沉淀。但反馈内容混在 `memory.md` 纯文本中，没有独立的结构化存储。反馈量增多后，检索和优先级判断会变差。

**目标**：用户反馈独立存储，支持按类别（纠正、偏好、表扬、抱怨）、时间、频率检索，与 memory/skill/project 体系解耦。

**待定**：存储格式（JSONL / SQLite）、是否纳入 self_audit 扫描、与现有 memory 的边界划分。

### 12.2 跨会话自动关联

**现状**：`session_search.py` 有三层混合搜索（BM25 + Embedding + 混合打分），能跨 session 检索历史。但这是被动搜索，需要 LLM 主动调 session 工具。不会自动关联"上次修了类似 bug"、"昨天处理过同类问题"。

**目标**：任务开始时自动检索近期相关历史，把相似上下文注入当前对话，减少重复踩坑。

**待定**：触发时机（每轮？仅复杂任务？）、注入方式（system prompt 额外层 / user message 前缀）、性能开销控制。
