...
def _write_skill_md(skill_dir: Path, name: str, description: str = "", triggers: list = None, body: str = "") -> Path:
...
class TestBuildSkillsIndex:
    ...
    def test_skills_index_contains_name_and_desc(self, tmp_path: Path):
        """索引包含 skill 名称和描述。"""
        f = _write_skill_md(tmp_path / "code-review", "code-review", "Review code quality", body="steps")
    ...
    def test_skills_index_format(self, tmp_path: Path):
        """索引格式是列表。"""
        f = _write_skill_md(tmp_path / "test", "test", "desc")
        mock_index = MagicMock()
...
class TestSkillIndexSearch:
    ...
    def test_search_finds_by_name(self, tmp_path: Path):
        """按名称搜索能找到。"""
        f = _write_skill_md(tmp_path / "debug", "debug", "Debug code")
        mock_index = MagicMock()
...
class TestSkillIndexIncrementalBuild:
    ...
    def test_new_skill_detected(self, tmp_path: Path):
        ...
        _write_skill_md(skills_dir / "skill-a", "skill-a", "desc a")
        idx = SkillIndex(skills_dir, index_dir)
        idx.load_or_build()

        _write_skill_md(skills_dir / "skill-b", "skill-b", "desc b")
    ...
    def test_modified_file_rebuilt(self, tmp_path: Path):
        ...
        _write_skill_md(skills_dir / "skill-a", "skill-a", "old desc")
        idx1 = SkillIndex(skills_dir, index_dir)
        idx1.load_or_build()

        time.sleep(0.1)
        _write_skill_md(skills_dir / "skill-a", "skill-a", "new desc")
        idx2 = SkillIndex(skills_dir, index_dir)
        idx2.load_or_build()
...
class TestSkillTriggerMerge:
    ...
    def test_parse_skill_without_triggers(self, tmp_path: Path):
        ...
        f = _write_skill_md(tmp_path / "test", "test", "desc", body="body")
        result = _parse_skill(f)
        assert result is not None
        assert result["name"] == "test"
